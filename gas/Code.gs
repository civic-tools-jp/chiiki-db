...
const RECORD_HEADERS=['id','branchId','areaId','contactId','lat','lng','fullAddress','personName','status','supporter','revisitPriority','type','date','memo','assigneeId','assigneeName','createdAt','updatedAt','updatedBy'];
const SESSION_HEADERS=['token','userId','expiresAt','createdAt'];

function doGet(){return json_({ok:true,name:'アイサポ Ver.2.1 API'});}
