...
function renderUsers(){$('usersTable').innerHTML=`<table class="admin-table admin-users-table"><thead><tr><th>ID</th><th>氏名</th><th>支部</th><th>権限/エリア</th><th>操作</th></tr></thead><tbody>${users.map(u=>`<tr>
<td data-label="ID">${esc(u.loginId)}</td>
<td data-label="氏名"><b>${esc(u.name)}</b></td>
<td data-label="支部">${esc(u.branchName)}</td>
<td data-label="権限/エリア"><div class="admin-user-role">${esc(roleLabel(u.role))}</div>${u.role==='member'?`<select id="ua_${esc(u.userId)}" class="mini-select admin-area-select">${areaOptionsForUser(u)}</select>`:'<span class="admin-area-note">エリア選択可</span>'}</td>
<td data-label="操作"><div class="admin-user-actions">${u.role==='member'?`<button class="mini-btn" onclick="saveUserArea('${esc(u.userId)}')">活動エリア変更</button>`:''}<button class="mini-btn" onclick="resetUserPassword('${esc(u.userId)}','${esc(u.loginId)}')">PWリセット</button></div></td>
</tr>`).join('')}</tbody></table>`}
