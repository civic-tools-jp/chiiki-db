"use strict";
function toggleLoginPassword(){const input=$('loginPassword'),btn=$('loginPasswordToggle');if(!input)return;const showing=input.type==='text';input.type=showing?'password':'text';if(btn){btn.setAttribute('aria-label',showing?'パスワードを表示':'パスワードを隠す');btn.title=showing?'パスワードを表示':'パスワードを隠す';btn.textContent=showing?'👁':'🙈';}}
